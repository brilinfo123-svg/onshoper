...
const IndiaAddressForm=({
  ...
  const selectLocation=(location)=>{
    ...
    // 🔥 IMPORTANT FIX
    setCurrentLocation(location); // <-- ADD THIS
  
    const stateCities=City.getCitiesOfState("IN", location.stateCode);
...
return (
  <div ref={formContainerRef} className={styles.container}>
    ...
    {/* 🔹 TOP LOCATION BOX (ALWAYS VISIBLE) */}
    {view === "state" && !showLocationSheet && (
      <div
        className={styles.currentLocationBox}
        onClick={() => setShowLocationSheet(true)}
      >
        <div className={styles.currentLocationIcon}>📍</div>
        ...
        <div className={styles.sheetContent}>
          ...
          <div className={styles.globalSearchWrapper}>
            <div className={styles.searchInputBox}>
              <span className={styles.searchIcon}><i className="icon-search-1"></i></span>
              ...
              <div className={styles.suggestionBox}>
                {globalSuggestions.map((location, index) => (
                  ...
                  >
                    ...
                    <div>
                      <h4>{location.area || location.city}</h4>
                      <p>
                        {location.city},{" "}
          ...
          {recentLocations.length > 0 && view === "state" && (
            <div className={`${styles.section} ${styles.RecentItemState}`}>
              ...
              {recentLocations.map((item, index) => (
                ...
                >
                  <span className={styles.recentIcon}><i className="icon-location"></i></span>

                  <div className={styles.LocationName}>
                     <p>{item.area || item.city}, </p><p>{item.city}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          >
            {/* STATES */}
            {view === "state" && (
              <div className={`${styles.section} ${styles.ChooseState}`}>
                <h5 className={styles.sectionTitle}>CHOOSE STATE</h5>
            ...
            {view === "city" && (
              <div className={`${styles.section} ${styles.CitySection}`}>
                ...
                {filteredCities.map((city) => {
                  const hasAreas =
                    Object.keys(
                      areaData?.[selectedState]?.[city.name] || {}
                    ).length > 0;
