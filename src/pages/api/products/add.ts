...
export default async function handler(
  ...
  form.parse(req, async (err, fields, files) => {
    ...
    try {
      ...
      // 👇 Extract shopOwnerID and contact
      const shopOwnerID = document.shopOwnerID;
      const contact = document.ownerEmail;
      const category = fields.category?.toString();
      ...
      // ✅ Mark product as free (no payment logic)
      document.isPaid = false;
      document.expiresAt = null;   // optional: remove expiry
      document.feature = false;    // optional: no featured flag
      ...
      return res.status(200).json({
        success: true,
        message: "Product listed successfully (free plan)",
