...
export function ChatProvider({ children }) {
  ...
  useEffect(() => {
    ...
    const handleReceiveMessage = (message) => {
      ...
      setChats((prev)=>{
        ...
        if(
        String(chat._id)!==String(message.chatId)
        ){
        return chat;
        }
      ...
      // If chat is open
      if (activeChat && String(message.chatId) === String(activeChat._id)) {
        setMessages((prev) => {
    ...
    const handleMessagesSeen = ({ chatId, seenAt }) => {
      console.log("👁 REALTIME SEEN", chatId, seenAt);
  ...
  useEffect(() => {
    ...
    if (session?.user?.id) loadChat();
  }, [router.isReady, router.query.chatId, session]);
  ...
  const startChat = async (user, product) => {
    try {
      ...
      router.push(`/chat?chatId=${data.chat._id}`);
    } catch (error) {
      console.log(error);
    }
  };
